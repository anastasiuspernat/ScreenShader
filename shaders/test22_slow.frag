...
float draw_scene(vec3 p)
	...
	float tube_p=2. ;  // +.2*(cos(p.z))  +tube_m;
	float tube_w=tube_p*0.091*abs(sin(time*0.5))  ;//  +0.0125*cos(PI*p.z*30.0);
	float tube=length(mod(p,tube_p)-tube_p*0.5)-tube_w;			// tube  => sphere change
		                                                ///////////////////////////////////////////
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
	...
	vec3 hit=ray+dir*t;
	vec2 h=vec2(-0.0025,0.002)*5.0; // light 5.0 is HDR mod by Anastasiy
	vec3 n=normalize(vec3(draw_scene(hit+h.xyx),draw_scene(hit+h.yxy),draw_scene(hit+h.yyx)))*1.5; //  2.0 is HDR mod by Anastasiy
