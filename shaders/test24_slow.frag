/*
 * Original shader from: https://glslsandbox.com/e#78747.0
 */
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ) {
    ...
    vec3 bc = mix(vec3(3.0, .0,.05), vec3(1.), bgShade);
    ...
    // Done.
	fragColor = vec4(clamp(sc, 0., 1.), sin(time/20.0)*3.0+1.0); // time is HDR mod by Anastasiy
